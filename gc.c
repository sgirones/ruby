...
again:
  if (LIKELY(objspace->mark_func_data == 0)) {
    obj = RANY(ptr);
    if (!is_markable_object(objspace, ptr)) goto exeunt;
    rgengc_check_relation(objspace, ptr);
    if (!gc_mark_ptr(objspace, ptr)) goto exeunt;  /* already marked */
  }
  else {
...
Init_GC(void)
{
  ...
  rb_define_module_function(rb_mObjSpace, "find_references", os_find_references, 1);
